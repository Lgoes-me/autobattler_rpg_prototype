        ...
        return id switch
        {
            ...
            ArchetypeIdentifier.Magos => new Archetype(id, currentAmount, new[] {1, 4, 6})
            {
                new OnSpecialAttackEventListener()
                {
                    (battle, user, ability) => 
                    {
                        var s2 = new StatsData(0,1,0,0);
                        var b2 = new StatModifierBuff(s2.ToDomain(), "Magos", -1, true);

                        GiveBuffToArchetypeInPlayerTeam(battle, b2, ArchetypeIdentifier.Magos);
                    }
                }
            },
            ...
            ArchetypeIdentifier.Herois => new Archetype(id, currentAmount, new[] {1, 8})
            {
                new OnBattleStartedListener()
                {
                    battle =>
                    {
                        var s2 = new StatsData(1,1,1,1);
                        var b2 = new StatModifierBuff(s2.ToDomain(), "Herois", -1);

                        GiveBuffToArchetypeInPlayerTeam(battle, b2, ArchetypeIdentifier.Herois);
                    }
                }
            },
