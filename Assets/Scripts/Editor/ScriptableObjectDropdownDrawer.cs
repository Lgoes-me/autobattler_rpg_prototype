...
public class ScriptableObjectDropdownDrawer : PropertyDrawer
{
    ...
    private static ScriptableObject[] GetScriptableObjects(SerializedProperty property)
    {
        ...
        {
            _scriptableObjects.Add(
                AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(guids[i]),
                    propertyType) as ScriptableObject);
        }
        ...
        SerializedProperty property, ScriptableObjectDropdownAttribute attribute)
    {
        ...
        else
        {
            EditorGUI.LabelField(position,
                "Use it with non-array ScriptableObject or derived class of ScriptableObject");
        }
    }
        ...
        SerializedProperty property, ScriptableObjectDropdownAttribute attribute)
    {
        ScriptableObject output = DrawScriptableObjectSelectionControl(position, label,
            property.objectReferenceValue as ScriptableObject, property, attribute);
        ...
        ScriptableObject scriptableObject, SerializedProperty property, ScriptableObjectDropdownAttribute attribute)
    {
        ...
        int controlID = GUIUtility.GetControlID(_controlHint, UnityEngine.FocusType.Keyboard, position);
    ...
    private static void DisplayDropDown(Rect position, ScriptableObject selectedScriptableObject,
        ScriptableObjectGrouping grouping)
    {
        ...
        for (int i = 0; i < _scriptableObjects.Count; ++i)
        {
            ...
            menu.AddItem(content, scriptableObject == selectedScriptableObject, _onSelectedScriptableObject,
                scriptableObject);
        }
